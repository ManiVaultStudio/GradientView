...
void PointPlotAction::initialize(GradientExplorerPlugin* scatterplotPlugin)
{
    ...
    connect(&_plugin->getPositionDataset(), &Dataset<Points>::changed, this, [this]() {

        const auto positionDataset = _plugin->getPositionDataset();
        ...
        const auto positionSourceDataset = _plugin->getPositionSourceDataset();
...
void PointPlotAction::updateDefaultDatasets()
{
    ...
    auto positionDataset = Dataset<Points>(_plugin->getPositionDataset());
...
void PointPlotAction::updateScatterPlotWidgetPointSizeScalars()
{
    ...
    const auto numberOfPoints = _plugin->getDataStore().getProjectionView().rows();
    ...
    if (_sizeAction.isSourceDataset()) {
        ...
        if (pointSizeSourceDataset.isValid() && pointSizeSourceDataset->getNumPoints() == _plugin->getPositionDataset()->getNumPoints())
        {
...
void PointPlotAction::updateScatterPlotWidgetPointOpacityScalars()
{
    ...
    if (!_plugin->getPositionDataset().isValid())
        return;

    _opacityAction.getMagnitudeAction().setEnabled(!_plugin->hasMaskApplied());

    if (_plugin->hasMaskApplied()) return;

    const auto numberOfPoints = _plugin->getPositionDataset()->getNumPoints();
    ...
    if (_opacityAction.isSourceDataset()) {
        ...
        if (pointOpacitySourceDataset.isValid() && pointOpacitySourceDataset->getNumPoints() == _plugin->getPositionDataset()->getNumPoints()) {
